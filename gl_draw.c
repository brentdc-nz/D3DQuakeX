...
#else
#ifdef D3DQUAKE
	// Marty - Updated the version string :)
	sprintf (ver, "(XBOX D3D Quake %4.2f) %4.2f", (float)GLQUAKE_VERSION, (float)VERSION);
	//sprintf (ver, "(d3d %4.2f, gl %4.2f) %4.2f", d3dGetD3DDriverVersion(), (float)GLQUAKE_VERSION, (float)VERSION); //MARTY
#else
...
void GL_Set2D (void)
{
	...
	glDisable (GL_BLEND);
	glEnable (GL_ALPHA_TEST);
//	glDisable (GL_ALPHA_TEST); //D3DQuake
