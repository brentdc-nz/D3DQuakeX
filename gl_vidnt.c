...
// MARTY - This is disabled in D3DQuake, why??
#if 0
...
//	VID_SetPalette (palette); //D3DQuake

//	gammaworks = SetDeviceGammaRamp (maindc, ramps); //D3DQuake
}
...
void d3dSetMode(/*int fullscreen,*/ int width, int height, int bpp, int zbpp, int vmode); //Marty 
#endif
...
void	VID_Init (unsigned char *palette)
{
...
#ifdef _XBOX // MARTY - Check if we are HD
	if(d3dIsResolutionHD())
	{
		// freakdave: also add 1280x720
		vid.width = 1280; 
		vid.height = 720;
	}
	else
...
#ifdef _XBOX //MARTY
		d3dSetMode(vid.width, vid.height, bpp, zbpp, true);
#else
    ...
    baseRC = wglCreateContext( /*maindc*/ ); //MARTY
	if (!baseRC)
	{
		Sys_Error ("Could not initialize GL (wglCreateContext failed).\n\nMake sure you in are 65535 color mode, and try running -window.");
	}
	if (!wglMakeCurrent(/* maindc, baseRC */)) //MARTY
	{	
		Sys_Error ("wglMakeCurrent failed");
	}
